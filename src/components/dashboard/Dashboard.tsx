...
const Dashboard: React.FC = () => {
  ...
  const stats = React.useMemo(() => {
    ...
    const todayTasks = allTasks.filter(task => task.due_date && task.due_date.startsWith(todayStr));
    const overdueTasks = allTasks.filter(task => !task.completed && task.due_date && new Date(task.due_date) < today);
    return {
  ...
  return <div className="h-full w-full p-6 pb-20 space-y-6">
      {/* Enhanced Header */}
      <motion.div initial={{
      opacity: 0,
      y: -20
          ...
          <CardContent className="space-y-4">
            <div className="space-y-3">
              ...
              <div className="w-full bg-white/10 rounded-full h-3 overflow-hidden">
                <motion.div className="bg-gradient-to-r from-green-400 via-blue-500 to-purple-500 h-3 rounded-full" initial={{
                width: 0
              }} animate={{
              ...
              <div className="flex justify-between items-center">
                ...
                {stats.overdue > 0 && <p className="text-red-300 text-caption font-medium">
