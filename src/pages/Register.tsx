...
export default function Register() {
  ...
  return <div className="min-h-screen flex">
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 bg-gradient-to-br from-gray-50 to-gray-100 bg-slate-950">
        <motion.div initial={{
        opacity: 0,
        y: 20
      }} animate={{
                ...
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                  ...
                  field
                        ...
                        <FormControl>
                          <div className="relative">
                            ...
                            <Input placeholder="Ad Soyadınız" className="pl-10" {...field} />
                          </div>
                        </FormControl>
                        <FormMessage />
                      </FormItem>} />
                  ...
                  field
                        ...
                        <FormControl>
                          <div className="relative">
                            ...
                            <Input type={showPassword ? 'text' : 'password'} placeholder="Şifrenizi oluşturun" className="pl-10 pr-10" {...field} />
                            <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-3 text-gray-400 hover:text-gray-600">
                              {showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                            </button>
                          </div>
                        </FormControl>
                        {watchedPassword && <div className="flex items-center justify-between text-xs">
                            <span className={passwordStrength.color}>
                              Şifre Gücü: {passwordStrength.feedback}
                            </span>
                            <div className="flex space-x-1">
                              {[1, 2, 3, 4, 5].map(level => <div key={level} className={`w-4 h-1 rounded ${level <= passwordStrength.score ? passwordStrength.score <= 2 ? 'bg-red-500' : passwordStrength.score <= 3 ? 'bg-yellow-500' : 'bg-green-500' : 'bg-gray-200'}`} />)}
                            </div>
                          </div>}
                        <FormMessage />
                      </FormItem>} />
                  ...
                  field
                        ...
                        <FormControl>
                          <div className="relative">
                            <Lock className="absolute left-3 top-3 h-4 w-4 text-gray-400" />
                            <Input type={showConfirmPassword ? 'text' : 'password'} placeholder="Şifrenizi tekrar girin" className="pl-10 pr-10" {...field} />
                  ...
                  <Button type="submit" className="w-full planmaster-button" disabled={isLoading}>
                    {isLoading ? <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-2" /> : <UserPlus className="w-4 h-4 mr-2" />}
                    Kayıt Ol
                  </Button>
                </form>
              </Form>
